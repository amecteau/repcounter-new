...
| Clear History | [features/clear-history-status.md](features/clear-history-status.md) | ✅ Complete (CH.1–CH.5) |
...
| 2026-04-27 | ESLint: relaxed `@typescript-eslint/no-unused-expressions` with `allowTernary: true` and `allowShortCircuit: true` | Project preference — `cond ? a() : b()` is a concise idiom for one-line control flow (e.g. `history.length > 1 ? history.back() : goto('/')`). Forcing it to an `if/else` block adds no clarity. Also allow short-circuit (`cond && callback?.()`) for the same reason. |
| 2026-04-29 | Elevated i18n to a Critical Rule in CLAUDE.md; added i18n checklist item to AGENTS.md Workflow Checklist; added i18n step to AGENTS.md "Adding a New Feature" | i18n was documented in AGENTS.md architecture but not surfaced as a per-task enforcement point. Hardcoded strings in components are the most likely violation in new features. |
| 2026-04-29 | Added "no optimistic updates on any data operation" rule to CLAUDE.md Critical Rules and AGENTS.md Error Handling | Any mutation (create, update, delete, clear) must commit to the data store before local state is updated. If the call fails, the UI must remain unchanged. |
| 2026-04-29 | Added "no elevated device permissions" rule to CLAUDE.md Critical Rules, new Platform Permissions Policy section in AGENTS.md, and two entries in AGENTS.md "What NOT to do" | File exports and all file I/O must use the OS-mediated dialog (SAF / document picker) to avoid WRITE_EXTERNAL_STORAGE and other sensitive permissions that trigger Google Play review and App Store scrutiny. |
